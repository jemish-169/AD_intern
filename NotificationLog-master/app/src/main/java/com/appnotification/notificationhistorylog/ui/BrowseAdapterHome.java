...
class BrowseAdapterHome extends  RecyclerView.Adapter<BrowseViewHolderHome>{
    ...
    private ArrayList<BrowseAdapterHome.DataItem> data = new ArrayList<>();
    private HashMap<String, Drawable> iconCache = new HashMap<>();
    private Handler handler = new Handler();
    private final List<UnifiedNativeAd> mRecyclerViewItems;

    private String lastDate = "";
    private boolean shouldLoadMore = true;

    BrowseAdapterHome(Activity context, List<UnifiedNativeAd> mNativeAds) {
    ...
    public void onBindViewHolder(@NonNull BrowseViewHolderHome vh, int position) {
        ...
        if (mRecyclerViewItems.size() > ads) {
            if (position % 18 == 0) {
                ...
                UnifiedNativeAd nativeAd = (UnifiedNativeAd) mRecyclerViewItems.get(ads);
                ads++;
...

        {
            for (BrowseAdapterHome.DataItem item: data){
                if (item.getAppName().toLowerCase().contains(text.toLowerCase())
                       ...
                       /* ||item.getAppName().toLowerCase().startsWith(text.toLowerCase())
                        ||item.getText().toLowerCase().startsWith(text.toLowerCase())*/
                        || item.getDate().toLowerCase().contains(text.toLowerCase())){
                    filteredList.add(item);
                }

                else if (item.getText().toLowerCase().contains(text.toLowerCase()))
                {
                    filteredList.add(item);

                }
                else if (item.getPackageName().toLowerCase().contains(text.toLowerCase()))
                {
                    filteredList.add(item);
